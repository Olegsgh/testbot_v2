...
@bot.message_handler(regexp="Отчёт \d{2}-\d{2}-\d{4}")
def send_welcome(message):
    response = reply_bot.reply_full_week_report(message)
    bot.reply_to(message, response)

@bot.message_handler(regexp="Отчёт \d{2}-\d{2}-\d{4} сумма")
def send_welcome(message):
    response = reply_bot.reply_kredit_week_report(message)
    bot.reply_to(message, response)

@bot.message_handler(regexp="Отчёт \d{2}-\d{2}-\d{4} сумма количество")
def send_welcome(message):
    response = reply_bot.reply_full_week_report(message)
    bot.reply_to(message, response)

@bot.message_handler(regexp="Отчёт \d{2}-\d{2}-\d{4} количество")
def send_welcome(message):
